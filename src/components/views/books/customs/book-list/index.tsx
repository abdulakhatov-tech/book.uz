...
const Aside: FC = () => {
	...
	return (
		<div
			className={`grid grid-cols-${limit} gap-${limit === 3 ? 6 : limit === 4 ? 5 : 3}`}
		>
			{loading
				? Array.from({ length: +(getParam("limit") as string) }).map(
						(_: any, idx: number) => <BookSkeleton key={idx} />,
					)
				: data?.map((book: BookI) => <ProductCard key={book._id} {...book} />)}
		</div>
	);
};
