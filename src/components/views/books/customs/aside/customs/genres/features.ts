...
const useGenresFeatures = () => {
  ...
  const [selectedGenres, setSelectedGenres] = useState<string[]>(initialSelectedGenres); // Init state with query params
  ...
  const filteredGenres = useMemo(() => {
    ...
    return data.filter((genre: GenreI) =>
      genre.name.toLowerCase().includes(search) // Filter by genre name
    );
  }, [data, search]);
