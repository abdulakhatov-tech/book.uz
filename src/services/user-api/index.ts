...
export const useUserApi = () => {
    ...
    const useGetDeliveryMethods = () => useQueryHandler({
    ...
    const useGetPaymentMethods = () => useQueryHandler({
    ...
    const useGetRegions = () => useQueryHandler({
        queryKey: ['regions'],
        queryFn: async () => {
    ...
    const useGetDistricts = (regionId: string) => useQueryHandler({
    ...
    const applyCoupon = useMutation({
        mutationFn: async (couponCode: string) => {
            const response = await axios.post('/user-api/apply-coupon', { couponCode });
            return response.data.data;
        }
    })
